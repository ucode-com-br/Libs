...
#if DEBUG
        protected internal static readonly ConcurrentDictionary<string, Type> CompiledMapperTypes = new();
#endif
...
#if DEBUG
        public static Type[] GetCompiledClonerTypes() => CompiledMapperTypes.ToArray().Select(kv => kv.Value).OrderBy(t => t.FullName).ToArray();

        public static string[] GetCompiledClonerTypeNames() => CompiledMapperTypes.ToArray().Select(kv => TypeNameResolver.Resolve(kv.Value)).OrderBy(n => n).ToArray();
#endif

        internal static bool IsSimpleValueType([NotNull] Type type)
        {
            if (!type.IsValueType)
            {
                return false;
